...
def _target_mask_tokens(
    ...
    return (
        spatial.clamp(0.0, 1.0)
        .reshape(1, -1)
        .repeat(1, generated_frames)
        .to(dtype=dtype)
    )


class MSRTargetRoutingAttentionOverride:
    """Gates generated-query access to each MSR reference token partition."""
    ...
    def __init__(
        ...
        if len(self.target_masks) != len(self.memory_token_counts):
            raise ValueError("Each MSR reference must have one target mask.")
    ...
    def _is_video_self_attention(self, q: torch.Tensor, k: torch.Tensor) -> bool:
        if q.ndim != 3 or k.ndim != 3 or q.shape[1] != k.shape[1]:
            return False
        return self.minimum_video_tokens <= q.shape[1] <= self.maximum_video_tokens
    ...
    def _routing_bias(
        ...
        for mask, reference_tokens in zip(
            self.target_masks,
            self.memory_token_counts,
        ):
            ...
            next_column = column + reference_tokens
            bias[
                :,
                :,
                : self.generated_token_count,

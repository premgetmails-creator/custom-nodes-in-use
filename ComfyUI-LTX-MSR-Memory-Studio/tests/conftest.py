...
PACKAGE_NAME = "ltx_msr_memory_studio_plugin"
...
class _FakeAddGuide:
    ...
    def add_keyframe_index(
        ...
        temporal = (
            torch.arange(frames).view(frames, 1).repeat(1, height * width).reshape(-1)
        )
        coords[:, 0, :, 0] = temporal + frame_idx
        coords[:, 0, :, 1] = temporal + frame_idx + 1
...
def _load_torch_file(path, safe_load=False, device=None, return_metadata=False):
    factor = "2" if "factor2" in str(path) else "1"
    payload = {"fake.lora_A.weight": torch.ones((1, 1))}
    metadata = {"reference_downscale_factor": factor}
...
def _load_lora_for_models(model, clip, lora, strength_model, strength_clip):
    output = model.clone()
    output.lora_strength = strength_model
    return output, clip
